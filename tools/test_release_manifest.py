...
def _policy_excluded(path: str) -> bool:
    """Ask the canonical policy, which is the authority on exclusion.

    The generated export lists concrete excluded paths only; it cannot answer for
    paths matched by an exclude glob (the four PGF fonts), and a manifest component
    for a policy-excluded path is legitimately absent from disk.
    """
    ...
    def test_manifest_integrity_all_entries_and_git_tracking(self):
        ...
        for comp in data.get("components", []):
            ...
            if presence == "tracked_file":
                ...
                if src_p in excluded or _policy_excluded(src_p):
                    continue
                full_src = ROOT / src_p
