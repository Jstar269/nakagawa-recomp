...
<#
.SYNOPSIS
    Fail-closed path/process/build safety primitives shared by nk_manager.ps1 and tests (#183, #196).

.DESCRIPTION
    Canonical successor to tools/hst_safety.ps1 (which is now a deprecated forwarding wrapper).
    ...
    evidence-corrupting:
      ...
      Assert-HstWorkspaceRoot      deprecated alias for Assert-NkWorkspaceRoot (backward compat).
...
function Assert-NkWorkspaceRoot {
    ...
    # Keep the legacy name in sync so that callers dot-sourcing this file and using
    # $script:HstWorkspaceRoot continue to work during the deprecation window.
    $script:HstWorkspaceRoot = $full
    return $full
}

# Backward-compat alias: deprecated, remove after Phase 5 rename sweep (#196).
Set-Alias -Name Assert-HstWorkspaceRoot -Value Assert-NkWorkspaceRoot -Scope Script
