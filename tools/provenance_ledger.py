...
def _validate_public_snapshot(
    ...
    *,
    ...
    if isinstance(refresh, dict):
        for field in ("trusted_tree", "candidate_tree"):
            recorded_tree = refresh.get(field)
            if recorded_tree is not None and recorded_tree != tree.tree_sha:
                raise RefreshError("TRUSTED_LEDGER_TREE_MISMATCH", f"{label} refresh metadata names a different tree")
