...
| P1 | Open | Exotic VFPU NaN/Inf matrix/multiply divergences: [issue #40](https://github.com/Jstar269/nakagawa-recomp/issues/40) is reduced to two mechanisms (NaN-payload operand order, now fixed; the vhdp ±0/NaN accumulation fold, now aligned with the interpreter's fold) and the codegen/interp split is fixed; PSP result bits remain hardware-unresolved pending the source-owned probes ([`fixtures/vfpu_nan_payload/`](fixtures/vfpu_nan_payload/), [`fixtures/vfpu_overlap_probe/`](fixtures/vfpu_overlap_probe/)). |
