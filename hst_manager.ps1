...
try {
    ...
    function Invoke-VerifySuite {
        ...
        Write-Host "`n[10/16] Checked-append formatting selftest (src/rt/strbuf_selftest.c)..." -ForegroundColor Cyan
        $a = $makeBaseArgs + @("strbuf-selftest", "--no-print-directory")
        $p = Start-Process -FilePath $makeExe -ArgumentList $a -PassThru -NoNewWindow -Wait
        if ($p.ExitCode -ne 0) { $failed += "strbuf-selftest" }
        Write-Host "`n[11/16] VFPU interpreter selftest (src/rt/vfpu_interp_selftest.c)..." -ForegroundColor Cyan
