...
/* Feature-test macro first: the environment probes below use setenv/unsetenv,
   which ISO C99 hides but POSIX.1-2001 declares. Windows uses _putenv. */
...
int main(int argc, char **argv) {
    ...
    /* 20. Host discovery: build preflight cards and the SDL3_ttf search order. */
    printf("[PLAYER_STATE_TEST] Subtest 20: build preflight cards and SDL3_ttf search order\n");
    fflush(stdout);
    {
        /* 20a: SDL3_ttf candidates put the executable folder first, then the
        ...
        /* 20b/20c: the two discovery cards a release user can hit. */
        ...
        /* 20c: back in the checkout, an empty PATH trips the toolchain gate
