...
def _cell_xstore() -> list[int]:
    """CROSS-TIER: the returning callee, omitted from the dispatch table.

    In lane MIXED this function has no native body, so the caller's `jalr` misses
    and the production interpreter floor executes it.  It stores, reads its own
    store back, and returns through `jr $ra` into the MIDDLE of a native frame
    that is still live -- the shape every existing AOT-gap gate avoids.
    """
    return [
        _i(0x09, ZERO, T1, 0x05A5),         # 0x00 addiu t1, zero, 0x5a5
        _i(0x2B, A0, T1, 0x20),             # 0x04 sw    t1, 0x20(a0)
        _i(0x23, A0, T2, 0x20),             # 0x08 lw    t2, 0x20(a0)  (own store)
        _i(0x09, V0, V0, 7),                # 0x0c addiu v0, v0, 7
        JR_RA,                              # 0x10 jr    ra -> interior of the caller
        _i(0x09, V0, V0, 1),                # 0x14 addiu v0, v0, 1 (slot)
    ]


def _cell_xcall() -> list[int]:
    """CROSS-TIER RETURNING CALL: AOT caller -> interpreted callee -> AOT resume.

    Generated code models `jalr` as a host CALL and emits `dispatch(s, _t);` with
    NO host return after it, so the caller's native frame resumes at its next
    statement once dispatch returns.  The continuation below reads back the word
    the callee stored and then bumps an accumulator, so the cell separates three
    distinct failures on plain architectural state:

      * the store not being visible across the tier boundary  -> $v1
      * the continuation running a different number of times   -> $v0
      * the frame being unwound more than once                 -> $sp / $ra
    """
    return [
        _i(0x09, SP, SP, -16),              # 0x00 addiu sp, sp, -16
        _i(0x2B, SP, RA, 12),               # 0x04 sw    ra, 12(sp)
        _i(0x09, ZERO, V0, 2),              # 0x08 addiu v0, zero, 2
        _i(0x0F, 0, T0, 0),                 # 0x0c lui   t0, %hi(xstore) (patched)
        _i(0x0D, T0, T0, 0),                # 0x10 ori   t0, t0, %lo(xstore)
        _r(T0, 0, RA, 0, 0x09),             # 0x14 jalr  ra, t0
        NOP,                                # 0x18 slot
        _i(0x23, A0, V1, 0x20),             # 0x1c lw    v1, 0x20(a0)  CROSS-TIER READ
        _i(0x09, V0, V0, 0x100),            # 0x20 addiu v0, v0, 256   continuation mark
        _i(0x23, SP, RA, 12),               # 0x24 lw    ra, 12(sp)
        JR_RA,                              # 0x28
        _i(0x09, SP, SP, 16),               # 0x2c addiu sp, sp, 16 (return slot)
    ]
...
def _cell_xtailmid() -> list[int]:
    """CROSS-TIER TAIL: the interpreted middle of a tail chain, also omitted.

    Unlike `xstore` this callee never returns: it stores and tail-transfers to a
    REGISTERED entry.  No native frame is left mid-flight, which is exactly the
    shape `fixtures/production_smoke` (aot-gap) and the platform ladder already
    use -- so this cell is the control that isolates guest-memory commitment from
    frame re-entry.
    """
    return [
        _i(0x09, ZERO, T1, 0x0C3C),         # 0x00 addiu t1, zero, 0xc3c
        _i(0x2B, A0, T1, 0x28),             # 0x04 sw    t1, 0x28(a0)
        _j(0x02, 0x00),                     # 0x08 j     xread (patched + relocated)
        _i(0x09, V0, V0, 1),                # 0x0c addiu v0, v0, 1 (slot)
    ]


def _cell_xread() -> list[int]:
    """CROSS-TIER TAIL destination: a REGISTERED entry that reads the store.

    Reached by the interpreter through a handoff in lane MIXED and by a direct
    compiled call in lane AOT.  It observes the word the previous (interpreted)
    tier committed, so the tail control asserts cross-tier memory visibility
    rather than merely surviving the transfer.
    """
    return [
        _i(0x23, A0, V1, 0x28),             # 0x00 lw    v1, 0x28(a0)  CROSS-TIER READ
        _i(0x09, V0, V0, 2),                # 0x04 addiu v0, v0, 2
        JR_RA,                              # 0x08
        NOP,                                # 0x0c
    ]


def _cell_xtail() -> list[int]:
    """CROSS-TIER tail entry: `jr $rs` into the omitted middle of the chain."""
    return [
        _i(0x09, ZERO, V0, 4),              # 0x00 addiu v0, zero, 4
        _i(0x0F, 0, T0, 0),                 # 0x04 lui   t0, %hi(xtailmid) (patched)
        _i(0x0D, T0, T0, 0),                # 0x08 ori   t0, t0, %lo(xtailmid)
        _r(T0, 0, 0, 0, 0x08),              # 0x0c jr    t0  (tail)
        NOP,                                # 0x10 slot
    ]
...
def _cell_negpad() -> list[int]:
    """Scratch canvas for the fail-closed negative corpus.

    Never called and never discovered, so no lane ever executes these bytes as
    part of a comparison.  The harness patches one rejected instruction sequence
    at a time into this extent and drives ``sr_guest_interp_run()`` at it
    directly, because a rejection is a property of ONE execution tier: the
    generated code is deliberately more permissive (``sr_oor()`` absorbs an
    out-of-range access instead of stopping), and making AOT manufacture matching
    faults just to fit a two-lane comparison would be fabricating agreement.

    It lives inside the fixture's ordinary ``.text`` extent so executable-span
    authority and arena readability are both real rather than simulated.
    """
    return [NOP] * 8
...
CELLS: tuple[tuple[str, str, object], ...] = (
    ...
    ("xstore", "cross-tier: the interpreted returning callee", _cell_xstore),
    ("xtailmid", "cross-tier: the interpreted tail middle", _cell_xtailmid),
    ("xread", "cross-tier: the registered tail destination", _cell_xread),
    ("xcall", "cross-tier RETURNING CALL across the AOT/interpreter seam",
     _cell_xcall),
...
NON_PROGRAM_CELLS = frozenset({"negpad"})

# Cells lane MIXED drops from the dispatch table, keyed by the entry cell that
# reaches them. Lane MIXED keeps every other native body, so one run crosses the
# tier boundary the production AOT-gap floor actually crosses.
CROSS_TIER_OMISSIONS: dict[str, str] = {
    "xcall": "xstore",
    "xtail": "xtailmid",
}
...
def build_text_segment() -> bytes:
    ...
    xstore = BASE + layout["xstore"][0]
    xtailmid = BASE + layout["xtailmid"][0]
    for cell, slot, address in (
        ...
        register = T1 if address == leaf_b else T0
    ...
    struct.pack_into(
        "<I", text, layout["xtailmid"][0] + 0x08, _j(0x02, layout["xread"][0])
    )
    return bytes(text)
...
def decode_form(word: int) -> tuple:
    """Classify one instruction the way BOTH execution tiers dispatch on it.

    Kept identical to tools/test_cosim_fixture.py's copy on purpose: the fixture
    inventory and the interpreter census must be expressed in the same vocabulary
    or comparing them would prove nothing.
    """
    primary = word >> 26
    if primary == 0x00:
...
def fixture_forms() -> set:
    """Every instruction form the cells actually execute."""
    layout = cell_layout()
    seen = set()
    ...
    for word in entry_words(layout):
        seen.add(decode_form(word))
    return seen


def _form_literal(form: tuple) -> str:
    if form[0] == "special":
        return f'X(SPECIAL, 0x{form[1]:02x}u, 0x00u, "special:0x{form[1]:02x}")'
    if form[0] == "cop1":
        return (
            f'X(COP1, 0x{form[1]:02x}u, 0x{form[2]:02x}u, '
...
def manifest_header() -> bytes:
    ...
    lines = [
        ...
        # Outside the flat arena entirely: sr_inrange_n() bounds guest PHYSICAL
        # addresses to [0, 0x0c000000), so 0x0d000000 is neither readable,
        ...
        f"#define COSIM_XTAILMID      0x{BASE + layout['xtailmid'][0]:08x}u",
        f"#define COSIM_XREAD         0x{BASE + layout['xread'][0]:08x}u",
        f"#define COSIM_NEGPAD        0x{BASE + layout['negpad'][0]:08x}u",
        f"#define COSIM_NEGPAD_WORDS  {len(layout['negpad'][1])}u",
        "",
        "/* X(name, guest_address, word_count, description, cross_tier_omit)",
        "   -- entry cells only. cross_tier_omit is 0 for an ordinary cell, or the",
        "   guest address lane MIXED must drop from the dispatch table. */",
    ...
    for index, (name, desc, (offset, words)) in enumerate(entries):
        ...
        omit = CROSS_TIER_OMISSIONS.get(name)
        omit_literal = f"0x{BASE + layout[omit][0]:08x}u" if omit else "0u"
        lines.append(
            f'    X({name}, 0x{BASE + offset:08x}u, {len(words)}u, "{desc}", '
            f'{omit_literal}){terminator}'
        )
    lines.extend([
        "",
        "/* The instruction forms the cells above actually execute.",
        "   X(kind, a, b, label): kind is PRIMARY, SPECIAL or COP1; a/b are the",
        "   discriminating fields. The harness probes the production interpreter for",
        "   the set it DECODES and requires the two to be equal, so neither side of",
        "   that claim is a hand-maintained list. */",
        "#define COSIM_FORM_LIST(X) \\",
    ])
    forms = sorted(fixture_forms())
    for index, form in enumerate(forms):
        terminator = "" if index == len(forms) - 1 else " \\"
        lines.append(f"    {_form_literal(form)}{terminator}")
    lines.append(f"#define COSIM_FORM_COUNT    {len(forms)}u")
        ...
        "cells": [
            {
                ...
                "cross_tier_omits": CROSS_TIER_OMISSIONS.get(name),
            }
...
def verify(build_dir: Path) -> int:
    ...
    # The cross-tier cells are only meaningful while their pointer pairs name the
    # cell lane MIXED actually drops; a generator change that desynchronised them
    # would make both lanes run the same tier and pass vacuously.
    for entry, target, slot in (("xcall", "xstore", 0x0C), ("xtail", "xtailmid", 0x04)):
        expected = BASE + layout[target][0]
        base = layout[entry][0] + slot
        hi = struct.unpack_from("<I", image, base)[0] & 0xFFFF
        lo = struct.unpack_from("<I", image, base + 4)[0] & 0xFFFF
        if ((hi << 16) | lo) != expected:
            problems.append(
                f"{entry} pointer pair materializes 0x{(hi << 16) | lo:08x}, "
                f"expected {target} at 0x{expected:08x}"
            )
    xtail_jump = struct.unpack_from("<I", image, layout["xtailmid"][0] + 0x08)[0]
    xtail_target = ((xtail_jump & 0x03FFFFFF) << 2) | (BASE & 0xF0000000)
    if (xtail_jump >> 26) != 2 or xtail_target != BASE + layout["xread"][0]:
        problems.append(
            f"xtailmid tail decodes to 0x{xtail_target:08x}, expected xread at "
            f"0x{BASE + layout['xread'][0]:08x}"
        )
