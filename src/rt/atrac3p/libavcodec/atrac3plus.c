...
/* LOCAL DELTA (Nakagawa recomp): upstream n4.4 calls avpriv_report_missing_feature()
 * (libavutil/internal.h:166, impl libavutil/log.c:483) without including a
 * declaring header; the standalone subset builds with
 * -Werror=implicit-function-declaration, so declare it via this header. */
#include "internal.h"
