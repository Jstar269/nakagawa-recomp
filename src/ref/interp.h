...
public:
	...
	// Emit a "# init" line mirroring the seeded CpuState, so a trace produced by
	// run_elf can serve as its own oracle: the codegen gate's driver re-seeds
	// the exact same initial register state from this line.
	void InitDump(const CpuState *s);
...
// Bit-reverse helper (Allegrex bitrev). Declared here so the interpreter and the
// runtime can share the same symbol; the definition lives in recomp.c.
