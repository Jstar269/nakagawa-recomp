...
1. `SR_DATA_EXPECTED_COUNT` in the build driver (`Makefile`, 56672) — a hard-coded
...
2. `SR_CALL_GUEST_STACK` (`src/rt/hle.c`, `src/rt/mpeg.c`) — one shared scratch stack
   for every nested guest call (see C-4).
3. `disc.id` duplication (C-3) and the remaining diagnostic-only groups above.
