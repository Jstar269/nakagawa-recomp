...
`docs/SETUP.md`, the environment doctor) is authoritative for current versions.
The JavaScript entries below are capture-time values from that branch; MSYS2/Vulkan
reference versions are capture-time values for
