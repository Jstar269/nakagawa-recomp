...
| `PREPARED_FOLDER_VALIDATION_WORKS` | Transactional staging & validation of game directory | **NO** (Manual external) | **YES** (`nk_core/prep_engine.py`) | **PASS** | Verified in `tools/test_nk_core.py::test_preparation_engine_transactional_flow` |
...
| `RUNTIME_PROCESS_STARTS` | Host launcher successfully spawns runtime binary | **YES** (PowerShell child) | Direct process spawn via platform API | **PASS** | `nk_platform_spawn_process` is wired through `nk_launch_prepare_session`; exercised by `tests/native/test_player_state.c` and by launching `display-smoke-v1` from the library |
