...
**Status: forward-looking plan + a first read-only tool.** This is Product 2
(decompilation) infrastructure. It does **not** touch the recompiler (`codegen.py`)
or the runtime. See [`ROADMAP.md`](ROADMAP.md) for how the three products relate and
[`PUBLICATION_READINESS.md`](PUBLICATION_READINESS.md) for why game-derived output is
private.
...
- The exporter writes only to the **gitignored build dir** and never uploads. Any
  ...
  public service.
- Matched source and symbols are **game data** → they live in the private game-data
  repo, not this one (see the "Repository topology" section of `ROADMAP.md`).
...
- [`ROADMAP.md`](ROADMAP.md) — the three products, repo topology, decompiler direction.
