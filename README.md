...
- **Decryption uses your own key file.** Most retail `EBOOT.BIN` and `.prx` files are encrypted. The player and `nk_cli` include a built-in decryption boundary that unwraps the disc's `EBOOT.BIN` when you supply your own local key file; the project ships no keys. Encrypted `.prx` modules still have to be supplied unencrypted ([#295](https://github.com/Jstar269/nakagawa-recomp/issues/295)).
