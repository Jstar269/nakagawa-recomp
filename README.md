...
- **PSP system fonts (optional but recommended for text):** Authentic in-game typography requires Sony firmware font files (`jpn0.pgf`, `ltn0.pgf`) dumped from a real PSP console (`flash0:/font/`). These proprietary files cannot legally be bundled and must be user-provided ([#300](https://github.com/Jstar269/nakagawa-recomp/issues/300)).
...
- **Missing compiled runtime:** If a game has been identified and staged but no matching valid v1 runtime package exists in `<player user data>/packages/<DISC_ID>/`, the card reports `RUNTIME_PACKAGE` as missing or incompatible, and Play remains disabled until a valid package is built ([#296](https://github.com/Jstar269/nakagawa-recomp/issues/296)/[#297](https://github.com/Jstar269/nakagawa-recomp/issues/297)).
- **Missing firmware fonts:** If in-game fonts cannot be found, the player notes that the PSP font `jpn0.pgf` is missing and must be user-provided, rather than substituting mismatched system fonts that cause text clipping (font provisioning in the works, [#300](https://github.com/Jstar269/nakagawa-recomp/issues/300)).
...
| System fonts | Partially works | Authentic typography requires user-provided Sony PSP firmware PGF fonts (`jpn0.pgf`, `ltn0.pgf`). Automated local font provisioning is in progress; fonts cannot be bundled. | [#300](https://github.com/Jstar269/nakagawa-recomp/issues/300) |
